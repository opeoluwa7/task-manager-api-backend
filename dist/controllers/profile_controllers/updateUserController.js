...
const updateUserController = async (express) => {
    try {
        const user_id = express.req.user?.user_id;
        const value = userSchema_1.updateUserSchema.safeParse(express.req.body);
        if (!value.success)
        ...
        if (email && email !== defaultEmail) {
            ...
            express.res.cookie('refresh_token', newRefreshToken, variables_1.refreshCookie);
        }
