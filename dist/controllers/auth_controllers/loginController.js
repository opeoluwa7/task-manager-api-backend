...
const loginController = async (express) => {
    try {
        const value = userSchema_1.loginSchema.safeParse(express.req.body);
        if (!value.success)
        ...
        express.res.cookie('refresh_token', refreshToken, variables_1.refreshCookie);
        express.res.cookie('access_token', accessToken, variables_1.accessCookie);
